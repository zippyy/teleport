...
func (h *Handler) githubCallback(w http.ResponseWriter, r *http.Request, p httprouter.Params) (interface{}, error) {
	...
	redirectURL, err := h.constructSSHResponseURL(AuthParams{
...
// constructSSHResponseURL creates a special SSH response for SSH login method
// that encodes everything using the client's secret key
func (h *Handler) constructSSHResponseURL(request AuthParams) (*url.URL, error) {
	u, err := url.Parse(request.ClientRedirectURL)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	sshLoginReq := services.SSHLoginEntryRequest{
		Username:    request.Username,
		Cert:        request.Cert,
		TLSCert:     request.TLSCert,
		HostSigners: services.AuthoritiesToTrustedCerts(request.HostSigners),
	}

	consoleResponse, err := h.cfg.ProxyClient.CreateSSHLoginEntry(sshLoginReq)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	out, err := services.GetSSHLoginEntryMarshaler().Marshal(consoleResponse)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	values := u.Query()
